...
void QrWidget::setQrCode(QrCode *qrCode) {
    if (m_qrcode) {
        delete m_qrcode;
    }
...
void QrWidget::paintEvent(QPaintEvent *event) {
    // Implementation adapted from Electrum: qrwidget.py
    if (!m_qrcode) {
        return;
    }
...
bool QrWidget::hasHeightForWidth() const {
    return true;
}

int QrWidget::heightForWidth(int w) const {
    return w;
}
